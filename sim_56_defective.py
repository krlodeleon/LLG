...
for internalSample_idx in range(n_samples_internal):
    # generating system and saving initial conditions
    systems = rd.MagneticSystem(N, n_samples, K) # generating system
    ...
    np.save(f'{dir_sys}/m0_{internalSample_idx}.npy', systems.m.get()) # saving m0
    ...
    np.save(f'{dir_sys}/J_{internalSample_idx}.npy', systems.J.get()) # saving coupling matrices
    ...
    np.save(f'{dir_sys}/history_N{N}_K{int(K)}_T{T_idx}_{sample_idx}_{internalSample_idx}.npy', history.get())
