#############################
# crydberg.py version 3.1.0 #
#############################

# Recycles J of thermal samples to save memory
...
class MagneticSystem:
    def __init__(self, N, n_samples_disorder, n_samples_thermal, B0=0, B0_theta=0, B0_phi=0, K=0):
        # setting number of system copies of the ensemble
        self.N = N
        self.n_samples_thermal = n_samples_thermal
        self.n_samples_disorder = n_samples_disorder
        self.n_networks = n_samples_thermal*n_samples_disorder

        # generating coupling matrix bank and ensemble coupling matrix as a view of the bank
        self.J_bank = cp.triu(cp.random.normal(0, 1/cp.sqrt(self.N), size=(self.n_samples_disorder, self.N, self.N)), 1)
        self.J_bank = self.J_bank + self.J_bank.transpose(0, 2, 1)
        self.J = cp.broadcast_to(self.J_bank[:,None,:,:], (self.n_samples_disorder, self.n_samples_thermal, N, N))

        # setting other dynamic parameters
    ...
    @property
    def m_Jcompatible(self):
        return self.m.reshape(self.n_samples_disorder, self.n_samples_thermal, self.N, 3)
    
    @property
    def energy_exchange(self):
        coupling_term = []
        for disorder_idx in range(self.n_samples_disorder):
            coupling_term.append((-self.J[disorder_idx] @ self.m_Jcompatible[disorder_idx]))
        Jtimesm = cp.array(coupling_term).reshape(self.m.shape)
        del coupling_term
        return (1/2)*cp.sum(self.m * Jtimesm, axis=(1,2))
    ...
    def B(self):
        ...
        for disorder_idx in range(self.n_samples_disorder):
            coupling_term.append((-self.J[disorder_idx] @ self.m_Jcompatible[disorder_idx]))
        B = cp.array(coupling_term).reshape(self.m.shape)
        del coupling_term
